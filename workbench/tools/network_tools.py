...
class NetworkTools:
    ...
    def __init__(self):
        self.ping_host_tool = StructuredTool.from_function(
            name="ping_host",
            func=self.ping_host,
            args_schema={
                "host": {
        ...
        self.download_file_tool = StructuredTool.from_function(
            name="download_file",
    ...
    def ping_host(self, host: str) -> str:
        ...
        try:
            ...
            command = ["ping", param, str(settings.PING_COUNT), host]

            # Run the ping command with timeout from config or default
            timeout = int(settings.WEB_REQUEST_TIMEOUT)
